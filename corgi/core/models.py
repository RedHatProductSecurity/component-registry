...
class Component(TimeStampedModel):
    ...
    def get_source(self):
        """return all ancestors with ComponentNode SOURCE type and descendant UPSTREAM"""
        source = []
        for ancestor in (
            self.cnodes.all().get_ancestors().filter(type=ComponentNode.ComponentNodeType.SOURCE)
        ):

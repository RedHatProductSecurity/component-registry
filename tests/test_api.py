...
@pytest.mark.django_db(databases=("default", "read_only"), transaction=True)
def test_srpm_component_url_params_provides_sources_upstreams(client, api_path):
    """
    ...
    * binary RPM never have child dependencies
    ...
    assert response.json()["count"] == 0
    response = client.get(f"{api_path}/components?re_provides={root_comp.name}")
    assert response.status_code == 200
    assert response.json()["results"] == []
    assert response.json()["count"] == 0
    ...
    assert response.status_code == 200
    assert response.json()["count"] == 1
    response = client.get(f"{api_path}/components?re_sources={root_comp.name}")
    assert response.status_code == 200
    assert response.json()["count"] == 1
    ...
    assert response.status_code == 200
    assert response.json()["count"] == 1
    response = client.get(f"{api_path}/components?re_provides={dep_comp.name}")
    assert response.status_code == 200
    assert response.json()["count"] == 1
    ...
    assert response.status_code == 200
    assert response.json()["count"] == 0
    response = client.get(f"{api_path}/components?re_sources={dep_comp.name}")
    assert response.status_code == 200
    assert response.json()["count"] == 0
